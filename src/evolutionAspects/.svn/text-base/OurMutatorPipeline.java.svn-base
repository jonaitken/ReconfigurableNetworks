...
public class OurMutatorPipeline extends BreedingPipeline
    ...
    public int produce(final int min, 
        ...
        long longMinGene=species.minGene(0);
        long longMaxGene=species.maxGene(0);
        // mutate 'em!
        for(int q=start;q<n+start;q++)
            {
            IntegerVectorIndividual i = (IntegerVectorIndividual)inds[q];
            for(int x=0;x<i.genome.length;x++) {
            	//System.err.println(species.mutationProbability);
            	//System.err.println(species.crossoverProbability);
            	
                if (state.random[thread].nextBoolean(species.mutationProbability)) {  
                	//Needs to be +2 to include end point as longMax is max address then +1 for -1
                	int newVal=state.random[thread].nextInt((int)longMaxGene+1+(-(int)longMinGene));
                	newVal-=1;
                	i.genome[x] = newVal;
                }
            }
